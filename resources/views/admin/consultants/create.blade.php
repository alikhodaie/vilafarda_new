@extends('layouts.admin.admin', ['title' => __('title.create consultant'), 'active' => 'consultants'])

@section('content')
    <x-admin.card title="{{ __('title.create consultant') }}">
        <form action="{{ route('admin.consultants.store') }}" method="POST" class="p-3 row" enctype="multipart/form-data">
            @csrf
            ...
            <div class="col-12 col-md-6 mb-5">
                ...
                <input class="form-control" name="phone_number" id="phone_number" type="text" value="{{ old('phone_number') }}"/>
            </div>

            <div class="col-12 col-md-6 mb-5">
                <label class="form-label" for="whatsapp_number">@lang('title.whatsapp_number')</label>
                <input class="form-control" name="whatsapp_number" id="whatsapp_number" type="text" value="{{ old('whatsapp_number') }}"/>
            </div>

            <div class="col-12 mb-5">
                <label class="form-label" for="whatsapp_default_message">@lang('title.whatsapp_default_message')</label>
                <input class="form-control" name="whatsapp_default_message" id="whatsapp_default_message" type="text" value="{{ old('whatsapp_default_message') }}">
            </div>
