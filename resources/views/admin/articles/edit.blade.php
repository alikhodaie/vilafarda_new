@extends('layouts.admin.admin', ['title' => __('title.edit article'), 'active' => 'articles'])

@section('content')
    <x-admin.card title="{{ __('title.edit article') }}">
        <form action="{{ route('admin.articles.update', $article->id) }}" enctype="multipart/form-data" method="POST" class="p-3 row">
            @csrf
            @method('PUT')
            ...
            <div class="col-12 col-md-6 mb-5">
                ...
                <input class="form-control" name="title" id="title" type="text" value="{{ old('title', $article->title) }}"/>
            </div>

            <div class="col-12 col-md-6 mb-5">
                <label class="form-label" for="slug">@lang('title.slug') <span>*</span></label>
                <input class="form-control" name="slug" id="slug" type="text" value="{{ old('slug', $article->slug) }}"/>
            </div>
                ...
                <select name="category" id="category" class="form-control">
                    ...
                    @else
                        ...
                        @foreach($items as $item)
                            <option value="{{ $item->id }}" @if(old('category', $article->categories->first()->id) == $item->id) selected @endif>{{ $item->title }} ({{ $item->id }})</option>
                        @endforeach
                    @endif
                </select>
            </div>
            ...
            <div class="col-12 mb-5">
                ...
                <textarea class="form-control" name="summary" id="summary" type="text">{!! old('summary', $article->summary) !!}</textarea>
            </div>
            ...
            <div class="col-12 col-md-6 mb-5">
                ...
                <meta-tag-input
                    ...
                    :values="{{ collect(old('metas', $article->meta)) }}"
                ></meta-tag-input>
            </div>
            ...
            <div class="col-12 col-md-6 mb-5">
                ...
                <tag-input
                    ...
                    no_result_text="@lang('text.empty_result')"
                    no_options_text="@lang('text.empty_list')"
                    :old="{{ \App\Models\Tag::query()->whereIn('id', old('tags', $article->tags->pluck('id')->toArray()))->get() }}"
            ...
            <div class="col-12 mt-5 d-flex justify-content-center">
                ...
                <a href="{{ route('admin.articles.index') }}" class="btn btn-falcon-danger">@lang('title.return')</a>
            </div>
        </form>
    </x-admin.card>
@endsection
