@extends('layouts.admin.admin', ['title' => __('title.edit category'), 'active' => 'article-categories'])

@section('content')
    <x-admin.card title="{{ __('title.edit category') }}">
        <form action="{{ route('admin.articles.categories.update', $category->id) }}" method="POST" class="p-3 row">
            @csrf
            @method('PUT')
