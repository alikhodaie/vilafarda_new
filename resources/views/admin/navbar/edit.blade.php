@extends('layouts.admin.admin', ['title' => __('title.edit navbar'), 'active' => 'navbar'])

@section('content')
    <x-admin.card title="{{ __('title.edit navbar') }}">
        <form action="{{ route('admin.navbar.update', $navbar->id) }}" method="POST" class="p-3 row">
            @csrf
            @method('PUT')

            <div class="col-12 mb-5">
                <label class="form-label" for="title">@lang('title.title') <span>*</span></label>
                <input class="form-control" name="title" id="title" type="text" value="{{ old('title', $navbar->title) }}"/>
            </div>

            <div class="col-12 mb-5">
                <label class="form-label" for="link">@lang('title.link') <span>*</span></label>
                <input class="form-control" name="link" id="link" type="url" value="{{ old('link', $navbar->link) }}"/>
            </div>

            <div class="col-12 mb-5">
                <label class="form-label" for="link">@lang('title.sort') <span>*</span></label>
                <input class="form-control" name="sort" min="0" id="sort" type="number" value="{{ old('sort', $navbar->sort) }}"/>
            </div>
                ...
                <select name="parent" id="parent" class="form-control">
                    ...
                    @else
                        ...
                        @foreach($items as $item)
                            <option value="{{ $item->id }}" @if($item->id == $navbar->parent_id) selected @endif>{{ $item->title }}({{ $item->id }})</option>
                        @endforeach
                    @endif
                </select>
            </div>
