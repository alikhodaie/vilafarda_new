@extends('layouts.admin.admin', ['title' => __('title.create category'), 'active' => 'faq-categories'])

@section('content')
    <x-admin.card title="{{ __('title.create category') }}">
        <form action="{{ route('admin.faq.categories.store') }}" method="POST" class="p-3 row">
            @csrf
