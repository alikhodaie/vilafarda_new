@extends('layouts.admin.admin', ['title' => __('title.homes').' | '.__('title.create option'), 'active' => 'homes-options'])
...
@section('content')
    <x-admin.card title="{{ __('title.create option') }}">
        <form action="{{ route('admin.homes.options.store') }}" enctype="multipart/form-data" method="POST" class="p-3 row">
            @csrf

            <x-admin.home-option-icon-field />
