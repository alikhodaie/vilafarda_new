@extends('layouts.admin.admin', ['title' => __('title.create health'), 'active' => 'homes-healths'])

@section('content')
    <x-admin.card title="{{ __('title.create health') }}">
        <form action="{{ route('admin.homes.healths.store') }}" method="POST" class="p-3 row">
            @csrf
