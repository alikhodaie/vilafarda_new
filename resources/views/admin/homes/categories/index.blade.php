@extends('layouts.admin.admin', ['title' => __('title.homes').' | '.__('title.categories'), 'active' => 'homes-categories'])

@section('content')
    <x-admin.search-card route="{{ route('admin.homes.categories.index') }}">
    ...
    <x-admin.card
        ...
        canSeeButton="{{ auth()->user()->can('createHome', \App\Models\Category::class) }}"
        buttonLink="{{ route('admin.homes.categories.create') }}">
                ...
                <table class="table table-hover table-striped overflow-hidden">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">@lang('title.homes count')</th>
                    ...
                    @foreach($categories as $category)
                        <tr class="align-middle">
                            ...
                            <td class="text-nowrap">{{ $category->homes_count }}</td>
                            <td class="text-end">
                                @can('updateHome', $category)
                                    <a href="{{ route('admin.homes.categories.edit', $category->id) }}" class="btn p-0" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('title.edit')">
                                        <span class="text-500 fas fa-edit"></span>
                                    </a>
                                @endcan
                                @can('deleteHome', $category)
                                    <delete-modal
                                        route="{{ route('admin.homes.categories.destroy', $category->id) }}"
                                        csrf="{{ csrf_token() }}"
