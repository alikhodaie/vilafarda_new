@extends('layouts.admin.admin', ['title' => __('title.edit category'), 'active' => 'homes-categories'])

@section('content')
    <x-admin.card title="{{ __('title.edit category') }}">
        <form action="{{ route('admin.homes.categories.update', $category->id) }}" method="POST" class="p-3 row">
            @csrf
            @method('PUT')
