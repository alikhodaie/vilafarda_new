@extends('layouts.admin.admin', ['title' => __('title.edit admin'), 'active' => 'admins'])

@section('content')
    <x-admin.card title="{{ __('title.edit admin') }}">
        <form action="{{ route('admin.admins.update', $admin->id) }}" method="POST" class="p-3 row" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="col-12 col-md-6 mb-3">
                ...
                <input class="form-control" name="first_name" id="first_name" type="text" value="{{ old('first_name', $admin->first_name) }}"/>
            </div>

            <div class="col-12 col-md-6 mb-3">
                <label class="form-label" for="last_name">@lang('title.last name') <span>*</span></label>
                <input class="form-control" name="last_name" id="last_name" type="text" value="{{ old('last_name', $admin->last_name) }}"/>
            </div>

            <div class="col-12 col-md-6 mb-3">
                <label class="form-label" for="mobile">@lang('title.mobile') <span>*</span></label>
                <input class="form-control" name="mobile" id="mobile" type="text" value="{{ old('mobile', $admin->mobile) }}"/>
            </div>

            <div class="col-12 col-md-6 mb-3">
                <label class="form-label" for="email">@lang('title.email')</label>
                <input class="form-control" name="email" id="email" type="email" value="{{ old('email', $admin->email) }}" placeholder="@lang('title.email')"/>
            </div>

            @can('adminUpdateRole', $admin)
                <div class="col-12 mb-3">
                    <label for="role">@lang('title.role') <span>*</span></label>
                    <select class="form-control" name="role" id="role">
                        <option value="">@lang('title.select')</option>
                        @foreach(\Spatie\Permission\Models\Role::all() as $role)
                            <option value="{{ $role->id }}" @if($role->id == old('role', $admin->roles->first()->id)) selected @endif>{{ $role->fa_name }}</option>
                        @endforeach
                    </select>
                </div>
            @endcan
            ...
            @can('adminBlock', $admin)
                <div class="col-12 mb-3">
                    <div class="form-check form-switch">
                        <input class="form-check-input" id="blocked" name="blocked" type="checkbox" @if(old('blocked', $admin->is_block)) checked="" @endif/>
